...
export function createDeltaSchema(language?: string) {
  ...
  return z.object({
    ...
    requirement: reqSchema.optional(),
    requirements: z.array(reqSchema).optional(),
    rename: z.object({
      from: z.string(),
      to: z.string(),
    }).optional(),
...
export const DeltaSchema = createDeltaSchema();

export function createChangeSchema(language?: string) {
  const deltaSchema = language ? createDeltaSchema(language) : DeltaSchema;
  return z.object({
...
export const ChangeSchema = createChangeSchema();
